...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function displayMatchesAsSchedule() {
        ...
        try {
            ...
            // NOVÉ: Načítame priradenia ubytovania
            ...
            // Spojíme všetky udalosti (zápasy, autobusy a ubytovania)
            ...
            allEvents.forEach(event => {
                ...
                } else if (event.type === 'accommodation') { // NOVÉ: Pridáme ubytovanie do unikátnych miest
            ...
            allEvents.forEach(event => {
                ...
                } else if (event.type === 'accommodation') { // NOVÉ: Pre ubytovanie predpokladáme, že trvá celý deň
                    startTimeInMinutes = 0 * 60; // Začiatok dňa
                    endTimeInMinutes = 24 * 60; // Koniec dňa
                }
            ...
            uniquePlacesForRows.forEach(placeData => { 
                ...
                sortedDates.forEach(date => {
                    ...
                    // Sort events for consistent display, matches first, then accommodations
                    ...
                    let currentVerticalOffset = 0; // To stack events
                        ...
                        if (event.type === 'match') {
                            ...
                            scheduleHtml += `
                                <div class="schedule-cell-match"
                                    ...
                                    style="left: ${matchBlockLeftPx}px; width: ${matchBlockWidthPx}px; top: ${currentVerticalOffset}px; height: ${ITEM_HEIGHT_PX}px;">
                            ...
                            if (event.bufferTime > 0) {
                                scheduleHtml += `
                                    <div class="schedule-cell-buffer"
                                        style="left: ${bufferBlockLeftPx}px; width: ${bufferBlockWidthPx}px; top: ${currentVerticalOffset}px; height: ${ITEM_HEIGHT_PX}px;">
                                    </div>
                                `;
                            }
                            currentVerticalOffset += ITEM_HEIGHT_PX + 5; // Stack vertically with a small gap
                        } else if (event.type === 'accommodation') { // NOVÉ: Vykreslenie ubytovania
                            const teamNames = event.teams.map(team => team.teamName).join(', ');
                            scheduleHtml += `
                                <div class="schedule-cell-accommodation"
                                    data-id="${event.id}" data-type="${event.type}"
                                    style="left: 0; width: 100%; top: ${currentVerticalOffset}px; height: 80px;">
                                    <div class="schedule-cell-content">
                            ...
                            `;
                            currentVerticalOffset += 80 + 5; // Stack vertically with a small gap
                        }
                    });
                    scheduleHtml += '</td>';
                });
            ...
            // NOVÉ: Event listener pre priradenie ubytovania v rozvrhu
