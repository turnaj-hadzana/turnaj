...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const matchModal = document.getElementById('matchModal');
    // Corrected selectors for close buttons based on the class 'close' within each modal
    const closeMatchModalButton = matchModal ? matchModal.querySelector('.close') : null;
    const matchForm = document.getElementById('matchForm');
    const matchIdInput = document.getElementById('matchId');
    ...
    const playingDayModal = document.getElementById('playingDayModal');
    const closePlayingDayModalButton = playingDayModal ? playingDayModal.querySelector('.close') : null;
    const playingDayForm = document.getElementById('playingDayForm');
    const playingDayIdInput = document.getElementById('playingDayId');
    const playingDayDateInput = document.getElementById('playingDayDate');
    ...
    const placeModal = document.getElementById('placeModal');
    const closePlaceModalButton = placeModal ? placeModal.querySelector('.close') : null;
    const placeForm = document.getElementById('placeForm');
    const placeIdInput = document.getElementById('placeId');
    ...
    addMatchButton.addEventListener('click', async () => {
        ...
        await populatePlacesSelect(matchLocationSelect); // Changed to populatePlacesSelect
        if (matchGroupSelect) {
            matchGroupSelect.innerHTML = '<option value="">-- Vyberte skupinu --</option>';
            matchGroupSelect.disabled = true;
        }
        team1Select.value = ''; // Changed to team1Select
        team2Select.value = ''; // Changed to team2Select
        ...
        if (matchCategorySelect.value) {
            // No need to await here, as updateMatchDurationAndBuffer is not async by itself in the common file.
            // If it becomes async, await will be needed.
            updateMatchDurationAndBuffer();
        } else {
    ...
    if (matchCategorySelect) {
        matchCategorySelect.addEventListener('change', async () => {
            ...
            if (selectedCategoryId) {
                ...
                // Update match duration and buffer based on selected category
                // For this to work, getCategoryMatchSettings (if it's in common.js) needs to be accessible
                // or the logic needs to be integrated here. Assuming it's available or will be added.
                // For now, I'm just calling it, and assuming it exists.
                // If the logic for updateMatchDurationAndBuffer is in spravca-turnaja-nastavenia.js,
                // it needs to be made accessible here, or its core logic copied/adapted.
                // Given the current setup, it's simpler to assume it's imported or globally available.
                // I will add a placeholder for it, if it's not present.
    ...
    async function getCategoryMatchSettingsAndUpdateInputs(categoryId) {
        ...
        matchBufferTimeInput.value = bufferTime;
        await findFirstAvailableTime(); // Recalculate time after updating duration/buffer
    }


    if (matchDateSelect) matchDateSelect.addEventListener('change', findFirstAvailableTime);
    if (matchLocationSelect) matchLocationSelect.addEventListener('change', findFirstAvailableTime);
    ...
    // Initial population of match form selects if modal is opened (e.g. for Add new match)
    // Removed redundant calls as they are handled by addMatchButton click listener
});

// The rest of your functions (getTeamName, calculateNextAvailableTime, moveAndRescheduleMatch,
// deletePlayingDay, deletePlace, editPlayingDay, editPlace, editMatch, deleteMatch)
// remain as they are, but their usage of global variables for DOM elements needs to be consistent
// with the new `const` declarations inside `DOMContentLoaded`.

// This function needs to be outside DOMContentLoaded or made globally accessible if used by editMatch
/**
 ...
 */
async function moveAndRescheduleMatch(draggedMatchId, targetPlayingDayId, targetPlaceId, droppedBeforeMatchId = null) {
    try {
        ...
        const playingDaysSnapshot = await getDocs(query(playingDaysCollectionRef, orderBy("date", "asc")));
        const sortedPlayingDays = playingDaysSnapshot.docs.map(d => d.data().date).sort();
        const targetPlayingDayData = playingDays.find(day => day.id === targetPlayingDayId); // Find actual date string
