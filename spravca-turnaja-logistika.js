...
async function openFreeSlotModal(date, location, startTime, endTime, blockedSlotId = null) {
    ...
    let blockButton = document.getElementById('blockSlotButton'); // Tlačidlo Zablokovať/Reblokovať
    let deleteUnblockButton = document.getElementById('deleteFreeSlotButton'); // Tlačidlo Odblokovať/Vymazať

    // Vyčistite všetky predchádzajúce poslucháče udalostí pre obe tlačidlá
    // Klonovanie a nahradenie elementu efektívne odstraňuje všetky predošlé poslucháče
    blockButton = blockButton.cloneNode(true);
    document.getElementById('blockSlotButton').replaceWith(blockButton);
    deleteUnblockButton = deleteUnblockButton.cloneNode(true);
    document.getElementById('deleteFreeSlotButton').replaceWith(deleteUnblockButton);
    ...
    if (blockedSlotId === null) {
        ...
        blockButton.addEventListener('click', () => createBlockedSlotAndRecalculate(date, location, startTime, endTime));
        ...
        deleteUnblockButton.addEventListener('click', () => { 
        ...
        blockButton.addEventListener('click', () => convertToRegularBlockedSlot(blockedSlotId, date, location));

        deleteUnblockButton.style.display = 'inline-block';
        deleteUnblockButton.textContent = 'Vymazať slot'; // Úplné vymazanie fantómu
        deleteUnblockButton.classList.add('delete-button');
        deleteUnblockButton.addEventListener('click', () => deleteSlotAndRecalculate(blockedSlotId, date, location));
        ...
        deleteUnblockButton.addEventListener('click', () => unblockBlockedSlot(blockedSlotId, date, location));
        ...
        blockButton.addEventListener('click', () => reblockUnblockedSlot(blockedSlotId, date, location));
    ...
    try {
        ...
        for (const date of allPlayingDayDates) {
            for (const location of allSportHalls) {
                console.log(`cleanupTrailingBlockedSlotsOnLoad: Kontrolujem dátum: ${date}, miesto: ${location}`); // OPRAVA: console.log
