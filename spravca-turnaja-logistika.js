    ...
    matchForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (overlapFound) {
                ...
                // To get place name, fetch from placesCollection
                ...
                const formattedDate = playingDayDoc.exists() ? `${String(new Date(playingDayDoc.data().date).getDate()).padStart(2, '0')}. ${String(new Date(playingDayDoc.data().date).getMonth() + 1).padStart(2, '0')}. ${new Date(playingDayDoc.data().date).getFullYear()}` : 'Neznámy dátum';
 ...
 * @param {string} hexColor The background color in hex (e.g., "#RRGGBB").
...
async function displayMatchesAsSchedule() {
    ...
    try {
        ...
        const matchesQuery = query(matchesCollectionRef, orderBy("playingDayId", "asc"), orderBy("placeId", "asc"), orderBy("time", "asc"));
        const matchesSnapshot = await getDocs(matchesQuery);
        ...
        const categoriesSnapshot = await getDocs(categoriesCollectionRef);
        const categoriesMap = new Map();
        const categoriesColorsMap = new Map(); // New map for colors
        categoriesSnapshot.forEach(doc => {
            const data = doc.data();
            categoriesMap.set(doc.id, data.name || doc.id);
            if (data.color) { // Assuming category documents have a 'color' field (e.g., "#RRGGBB")
                categoriesColorsMap.set(doc.id, data.color);
            }
        });
        console.log("displayMatchesAsSchedule: Načítané kategórie:", Array.from(categoriesMap.entries()));
        console.log("displayMatchesAsSchedule: Načítané farby kategórií:", Array.from(categoriesColorsMap.entries()));
        ...
        const playingDaysSnapshot = await getDocs(query(playingDaysCollectionRef, orderBy("date", "asc")));
        const existingPlayingDays = playingDaysSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        const playingDaysMap = new Map(); // Map for easy lookup
        existingPlayingDays.forEach(day => playingDaysMap.set(day.id, day));
        console.log("displayMatchesAsSchedule: Načítané hracie dni:", existingPlayingDays);
        ...
        const placesMap = new Map(); // Map for easy lookup
        existingPlacesData.forEach(place => placesMap.set(place.id, place));
        console.log("displayMatchesAsSchedule: Načítané miesta:", existingPlacesData);


        // Group matches first by Place, then by Date
        const groupedMatchesByPlace = new Map(); // Key: "placeId", Value: Map (Key: "playingDayId", Value: Array of matches)
        allMatches.forEach(match => {
            // Only show matches that have valid playingDayId and placeId, and are linked to a sport hall
            const place = placesMap.get(match.placeId);
            if (match.playingDayId && match.placeId && place && place.type === 'Športová hala') {
                const placeId = match.placeId;
                const playingDayId = match.playingDayId;

                if (!groupedMatchesByPlace.has(placeId)) {
                    groupedMatchesByPlace.set(placeId, new Map());
                }
                const matchesByDay = groupedMatchesByPlace.get(placeId);
                if (!matchesByDay.has(playingDayId)) {
                    matchesByDay.set(playingDayId, []);
                }
                matchesByDay.get(playingDayId).push(match);
            }
        });
        ...
        const sortedPlaces = Array.from(placesMap.values()).filter(p => p.type === 'Športová hala').sort((a, b) => a.name.localeCompare(b.name));

        if (allMatches.length === 0) { // Check if there are any matches in total
            scheduleHtml += '<p>Žiadne zápasy na zobrazenie. Pridajte nové zápasy pomocou tlačidla "+".</p>';
        } else if (sortedPlaces.length === 0) { // If there are matches but no sport halls
            scheduleHtml += '<p>Žiadne športové haly na zobrazenie. Skontrolujte nastavenia miest.</p>';
        } else {
            sortedPlaces.forEach(place => {
                const placeName = place.name;
                const placeId = place.id;
                const matchesByDayForPlace = groupedMatchesByPlace.get(placeId) || new Map(); // Get matches for this place, or empty map if none

                // Sort dates within each location based on actual date string
                const sortedDatesForPlace = Array.from(matchesByDayForPlace.keys()).sort((aId, bId) => {
                    const dateA = playingDaysMap.get(aId)?.date;
                    const dateB = playingDaysMap.get(bId)?.date;
                    return dateA ? dateA.localeCompare(dateB) : 0;
                });

                // Add playing days that have no matches for this place
                const allPlayingDayIds = existingPlayingDays.map(day => day.id);
                const missingPlayingDayIds = allPlayingDayIds.filter(dayId => !matchesByDayForPlace.has(dayId));
                missingPlayingDayIds.forEach(dayId => {
                    matchesByDayForPlace.set(dayId, []); // Add empty array for days without matches
                    if (!sortedDatesForPlace.includes(dayId)) {
                        sortedDatesForPlace.push(dayId); // Add to sorted list
                    }
                });
                sortedDatesForPlace.sort((aId, bId) => { // Re-sort to include newly added empty days
                    const dateA = playingDaysMap.get(aId)?.date;
                    const dateB = playingDaysMap.get(bId)?.date;
                    return dateA ? dateA.localeCompare(dateB) : 0;
                });
                ...
                scheduleHtml += `<h2 style="background-color: #007bff; color: white; padding: 18px; margin: 0; text-align: center;">${placeName}</h2>`;

                sortedDatesForPlace.forEach(playingDayId => {
                    const matchesForDayAndPlace = matchesByDayForPlace.get(playingDayId);
                    const playingDayData = playingDaysMap.get(playingDayId); // Get full playing day data

                    // Sort matches by start time
                    if (matchesForDayAndPlace) {
                        matchesForDayAndPlace.sort((a, b) => {
                    ...
                    const displayDate = playingDayData ? playingDayData.date : 'Neznámy dátum';
                    ...
                    if (matchesForDayAndPlace && matchesForDayAndPlace.length > 0) {
                        matchesForDayAndPlace.forEach(match => {
                            ...
                            const categoryColor = categoriesColorsMap.get(match.categoryId) || '#CCCCCC'; // Default grey
                        ...
                        scheduleHtml += `<tr><td colspan="5" style="text-align: center; color: #666;">Žiadne zápasy pre tento deň a miesto.</td></tr>`;
                    }
        ...
        matchesContainer.querySelectorAll('.match-row').forEach(row => {
            row.addEventListener('click', (event) => {
                const matchId = event.currentTarget.dataset.matchId; // Corrected to match data-match-id
                editMatch(matchId);
            });
            // Add dragstart listener
            row.addEventListener('dragstart', (event) => {
                event.dataTransfer.setData('text/plain', event.target.dataset.matchId); // Corrected to match data-match-id
            ...
            row.addEventListener('drop', async (event) => {
                ...
                const newPlayingDayId = parentDateGroup.dataset.playingDayId; // Corrected to playing-day-id
                const newPlaceId = parentDateGroup.dataset.placeId; // Corrected to place-id

                if (draggedMatchId && newPlayingDayId && newPlaceId && targetMatchId) {
                    await moveAndRescheduleMatch(draggedMatchId, newPlayingDayId, newPlaceId, targetMatchId);
                }
            });
        });
        ...
        matchesContainer.querySelectorAll('.date-group').forEach(dateGroupDiv => {
            ...
            dateGroupDiv.addEventListener('drop', async (event) => {
                ...
                const draggedMatchId = event.dataTransfer.getData('text/plain');
                const newPlayingDayId = dateGroupDiv.dataset.playingDayId; // Corrected to playing-day-id
                const newPlaceId = dateGroupDiv.dataset.placeId; // Corrected to place-id
                ...
                if (draggedMatchId && newPlayingDayId && newPlaceId) {
                    // If dropped directly on the date-group, it means append to end or find first available
                    await moveAndRescheduleMatch(draggedMatchId, newPlayingDayId, newPlaceId, null); // null indicates append to end
                }
            });
        });
        ...
        // These click listeners for date/location headers are now part of the `displayMatchesAsSchedule`
        // and should refer to `playingDayId` and `placeId` for the correct mapping to Firestore documents.

        matchesContainer.querySelectorAll('.date-group h3').forEach(header => {
            header.addEventListener('click', (event) => {
                // Ensure the click is on the header itself, not its children like buttons/links
                if (event.target === header || event.target.closest('h3') === header) {
                    const playingDayIdToEdit = header.closest('.date-group').dataset.playingDayId;
                    // Need to fetch playing day data by ID to pass to openPlayingDayModal
                    (async () => {
                        try {
                            const dayDoc = await getDoc(doc(playingDaysCollectionRef, playingDayIdToEdit));
                            if (dayDoc.exists()) {
                                openPlayingDayModal(dayDoc.id, dayDoc.data());
                            } else {
                                showMessage('Informácia', 'Hrací deň sa nenašiel.');
                            }
                        } catch (error) {
                            console.error('Chyba pri načítaní dát hracieho dňa pre úpravu:', error);
                            showMessage('Chyba', 'Chyba pri načítaní dát hracieho dňa pre úpravu.');
                        }
                    })();
                }
            });
        ...
        // Click listener for the main place/location header (h2 element)
        matchesContainer.querySelectorAll('.location-group h2').forEach(header => {
            header.addEventListener('click', (event) => {
                if (event.target === header || event.target.closest('h2') === header) {
                    const placeIdToEdit = header.closest('.location-group').dataset.placeId; // Assuming location-group has data-place-id
                    (async () => {
                        try {
                            const placeDoc = await getDoc(doc(placesCollectionRef, placeIdToEdit));
                            if (placeDoc.exists()) {
                                openPlaceModal(placeDoc.id, placeDoc.data());
                            } else {
                                showMessage('Informácia', 'Miesto sa nenašlo.');
                            }
                        } catch (error) {
                            console.error('Chyba pri načítaní dát miesta pre úpravu:', error);
                            showMessage('Chyba', 'Chyba pri načítaní dát miesta pre úpravu.');
                        }
                    })();
                }
            });
...
async function deletePlayingDay(playingDayId) {
    // Fetch playing day data to get its date for the confirmation message
    const playingDayDoc = await getDoc(doc(playingDaysCollectionRef, playingDayId));
    const dateToDelete = playingDayDoc.exists() ? playingDayDoc.data().date : 'neznámy dátum';
...
async function deletePlace(placeIdToDelete) {
    // Fetch place data to get its name and type for the confirmation message
    const placeDoc = await getDoc(doc(placesCollectionRef, placeIdToDelete));
    const placeData = placeDoc.exists() ? placeDoc.data() : { name: 'neznáme miesto', type: 'neznámy typ' };

    const confirmed = await showConfirmation(
        'Potvrdenie vymazania',
        `Naozaj chcete vymazať miesto ${placeData.name} (${placeData.type}) a VŠETKY zápasy, ktoré sa viažu na toto miesto?`
    );
    ...
    if (confirmed) {
        try {
            ...
            await batch.commit();
            await showMessage('Úspech', `Miesto ${placeData.name} (${placeData.type}) a všetky súvisiace zápasy boli vymazané!`);
            closeModal(document.getElementById('placeModal'));
            await displayPlacesTable();
            await displayMatchesAsSchedule();
        } catch (error) {
            console.error("Chyba pri mazaní miesta:", error);
            await showMessage('Chyba', `Chyba pri mazaní miesta ${placeData.name} (${placeData.type}). Detail: ${error.message}`);
        }
    }
}
 ...
 * @param {object} playingDayData The data of the playing day to edit.
 */
async function editPlayingDay(playingDayId, playingDayData) {
    try {
        const playingDayModal = document.getElementById('playingDayModal');
        const playingDayIdInput = document.getElementById('playingDayId');
        const playingDayDateInput = document.getElementById('playingDayDate');
        ...
        deletePlayingDayButtonModal.onclick = () => deletePlayingDay(playingDayId); // Pass ID, not date
        openModal(playingDayModal);
 ...
 * @param {object} placeData The data of the place to edit.
 */
async function editPlace(placeId, placeData) {
    try {
        const placeModal = document.getElementById('placeModal');
        const placeIdInput = document.getElementById('placeId');
        const placeTypeSelect = document.getElementById('placeTypeSelect');
        ...
        deletePlaceButtonModal.onclick = () => deletePlace(placeId); // Pass ID, not name/type
        openModal(placeModal);
    } catch (error) {
 ...
 * @param {string} [newPlayingDayId=''] Optional: New playingDayId to pre-fill the modal with.
 * @param {string} [newPlaceId=''] Optional: New placeId to pre-fill the modal with.
 */
async function editMatch(matchId, newPlayingDayId = '', newPlaceId = '') {
    try {
        ...
        const matchPlayingDaySelect = document.getElementById('matchPlayingDaySelect'); // Corrected ID
        const matchPlaceSelect = document.getElementById('matchPlaceSelect'); // Corrected ID
        const matchTimeInput = document.getElementById('matchTimeInput'); // Corrected ID
        const matchDurationInput = document.getElementById('matchDuration');
        const matchBufferTimeInput = document.getElementById('matchBufferTime');
        const matchCategorySelect = document.getElementById('matchCategorySelect'); // Corrected ID
        const matchGroupSelect = document.getElementById('matchGroupSelect'); // Corrected ID
        const team1Select = document.getElementById('team1Select'); // Corrected ID
        const team2Select = document.getElementById('team2Select'); // Corrected ID
        const deleteMatchButtonModal = document.getElementById('deleteMatchButtonModal'); // Corrected ID
        ...
        if (matchDoc.exists()) {
            ...
            await populatePlacesSelect(matchPlaceSelect, newPlaceId || matchData.placeId); // Use placeId here
            ...
            if (matchData.categoryId) {
                await populateGroupSelect(matchGroupSelect, matchData.categoryId, matchData.groupId); // Changed order of args
                matchGroupSelect.disabled = false;
            } else {
                matchGroupSelect.innerHTML = '<option value="">-- Vyberte skupinu --</option>';
                matchGroupSelect.disabled = true;
            ...
            // Populate teams using their IDs
            await populateTeamsForGroupSelect(team1Select, matchData.groupId, matchData.team1Id);
            await populateTeamsForGroupSelect(team2Select, matchData.groupId, matchData.team2Id);
            ...
            if (newPlayingDayId || newPlaceId) { // Check for new IDs, not just values
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const matchModal = document.getElementById('matchModal');
    const closeMatchModalButton = matchModal ? matchModal.querySelector('.close') : null; // Corrected
    const matchForm = document.getElementById('matchForm');
    const matchIdInput = document.getElementById('matchId');
    const matchPlayingDaySelect = document.getElementById('matchPlayingDaySelect'); // Corrected ID
    const matchPlaceSelect = document.getElementById('matchPlaceSelect'); // Corrected ID
    ...
    const playingDayModal = document.getElementById('playingDayModal');
    const closePlayingDayModalButton = playingDayModal ? playingDayModal.querySelector('.close') : null; // Corrected
    const playingDayForm = document.getElementById('playingDayForm');
    const playingDayIdInput = document.getElementById('playingDayId');
    const playingDayDateInput = document.getElementById('playingDayDate');
    const playingDayModalTitle = document.getElementById('playingDayModalTitle');
    ...
    const placeModal = document.getElementById('placeModal');
    const closePlaceModalButton = placeModal ? placeModal.querySelector('.close') : null; // Corrected
    const placeForm = document.getElementById('placeForm');
    const placeIdInput = document.getElementById('placeId');
    const placeTypeSelect = document.getElementById('placeTypeSelect');
    const placeNameInput = document.getElementById('placeName');
