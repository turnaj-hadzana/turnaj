...
async function displayMatchesAsSchedule() {
    ...
    try {
            ...
            for (const placeId in dayData.places) {
                ...
                if (placeData.matches.length === 0) {
                    ...
                    placeData.matches.forEach((match, index) => {
                        ...
                        const team1 = allClubs.find(club => club.id === match.team1Id);
                        const team2 = allClubs.find(club => club.id === match.team2Id);
                        const category = allCategories.find(cat => cat.id === match.categoryId);
                        const group = allGroups.find(grp => grp.id === match.groupId);

                        const team1Name = team1 ? team1.clubName : 'Neznámy tím 1';
                        const team2Name = team2 ? team2.clubName : 'Neznámy tím 2';
                        const categoryName = category ? category.name : 'Neznáma kategória';
                        const groupName = group ? group.name : 'Neznáma skupina';

                        teamsTd.innerHTML = `
                            ${team1Name} vs ${team2Name}<br>
                            <small>${categoryName} - Skupina ${groupName}</small>
                        `;

                        // Apply background color based on category, if available
                        if (category && category.color) {
                            teamsTd.style.backgroundColor = category.color;
                            // Optionally, set text color to white for better readability on dark backgrounds
                            // A simple heuristic for dark vs light color: sum of RGB values or luminance
                            const hex = category.color.substring(1); // Remove #
                            const r = parseInt(hex.substring(0, 2), 16);
                            const g = parseInt(hex.substring(2, 4), 16);
                            const b = parseInt(hex.substring(4, 6), 16);
                            // Calculate luminance (YIQ method)
                            const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
                            if (luminance < 0.5) { // If dark color, set text to white
                                teamsTd.style.color = 'white';
                            } else {
                                teamsTd.style.color = '#333'; // Default text color for light backgrounds
                            }
                        }
        ...
        scheduleTableBody.innerHTML = '<tr><td colspan="5" style="text-align: center; color: red;">Chyba pri načítavaní rozpisu zápasov.</td></tr>';
    }
}
