// spravca-turnaja-zoznam-timov.js (Celý kód s úpravami pre interaktívny filter)
...
// Referencie pre filtračnú časť modálu
...
let teamsToDisplay = []; // Pole tímov, ktoré sa majú aktuálne zobraziť v tabuľke (výsledok filtrovania)
...
// ZMENA: currentFilter môže ukladať viacero filtrov, ale pre zjednodušenie interaktivity ho necháme na jeden.
// Filtrovanie bude prebiehať sekvenčne v displayCreatedTeams.
// Hodnota bude uložená ako { type: 'teamName'|'category'|'group', value: 'hodnota_filtra'|null }
...
// Získa unikátne základné názvy tímov pre filter "Názov tímu" Z AKTULNE ZOBRAZENÝCH TÍMOV
...
// Získa unikátne názvy kategórií pre filter "Kategória" Z AKTULNE ZOBRAZENÝCH TÍMOV
function getUniqueTeamCategories(teams, categories) { // teams bude teamsToDisplay
    // Získa unikátne categoryId z tímov, ktoré nie sú null alebo undefined
    const categoryIds = [...new Set(teams.map(team => team.categoryId).filter(id => id !== null && typeof id !== 'undefined'))];
    ...
    const categoryNames = categoryIds.map(id => {
     ...
     // Vráti unikátne, zoradené názvy kategórií, filtruje prázdne reťazce okrem "Neznáma kategória" ak sa vyskytne ako skutočná hodnota ID
     return [...new Set(categoryNames.filter(name => name && name.trim() !== ''))]
        .sort((a, b) => a.localeCompare(b, 'sk-SK'));
}

// Získa unikátne názvy skupín pre filter "Skupina" Z AKTULNE ZOBRAZENÝCH TÍMOV
function getUniqueTeamGroups(teams, groups) { // teams bude teamsToDisplay
    const groupNames = new Set();

    teams.forEach(team => {
        // ZMENA: Skontrolovať len groupId, nie assignedGroup
        if (team.groupId === null || typeof team.groupId === 'undefined' || team.groupId === '') {
...
function populateGroupSelectForClubModal(selectElement, selectedId = '', availableGroups, categoryId = null) {
     ...
     // ZMENA: Filtrovať skupiny, ktorých categoryId sa zhoduje s categoryId tímu
...
function resetClubModal() {
     ...
     // editingClubId a currentClubModalMode sa nastavujú priamo v openClubModal, tu sa len resetujú na null
     editingClubId = null;
     ...
     if (clubCategorySelect) {
         ...
         clubCategorySelect.onchange = null; // Odstrániť event listener
     }
     ...
     if (clubGroupSelect) {
         ...
         if (clubGroupSelect) clubGroupSelect.onchange = null; // Odstrániť event listener
     }
     ...
     if (unassignedClubSelect) {
         ...
         unassignedClubSelect.onchange = null; // Odstrániť event listener
     }
     ...
     // Zabezpečiť, že povinné atribúty sú správne nastavené pre predvolený mód (Assign/Edit - len názov)
     // clubGroupSelect a orderInGroupInput by nemali byť predvolene required, lebo tím nemusí byť priradený
     // Tieto atribúty sa nastavujú podľa potreby v openClubModal pre špecifické režimy (napr. Assign)
     ...
     // Resetovať filtračný select a titulok
     if (filterModalTitle) filterModalTitle.textContent = 'Filter';
     if (filterSelect) {
         filterSelect.innerHTML = '<option value="">-- Zobraziť všetko --</option>';
         filterSelect.onchange = null; // Odstrániť event listener
         filterSelect.value = ""; // Zabezpečiť, že je vybraný placeholder
     }

    // POZOR: currentFilter NIE JE resetovaný v resetClubModal!
    // Resetuje sa len pri výbere '-- Zobraziť všetko --' vo filtračnom modále
    // currentFilter = { type: null, value: null }; // Ak by sme chceli resetovať filter pri zatvorení modalu úpravy/priradenia
}
...
async function openClubModal(identifier = null, mode = 'assign') {
     console.log(`Otváram modál Klub v režime: ${mode}, Identifier: ${identifier}`);
    ...
    // Resetovať modál pred otvorením (nechá currentFilter nedotknutý)
    ...
    currentClubModalMode = mode; // Uložiť aktuálny režim modálu
     ...
     if (['assign', 'edit', 'create'].includes(mode)) {
             ...
             if (unassignedClubSelect) {
                  unassignedClubSelect.onchange = () => {
                       ...
                       if (selectedId && categoryId && !categoryId.startsWith('--')) {
                            ...
                            if (clubGroupSelect) clubGroupSelect.disabled = true;
                            populateGroupSelectForClubModal(clubGroupSelect, null, allAvailableGroups, null); // Resetovať skupiny (zobraziť všetky alebo placeholder)
         ...
         // Logika pre režim "Upraviť" (edit)
             ...
             clubModalTitle.textContent = 'Upraviť tím / Priradiť klub'; // Titulok môže zostať univerzálny
             ...
             // Odstrániť listenery z iných módov, ak boli pridané
             if (unassignedClubSelect) unassignedClubSelect.onchange = null;
              // Listenery na zmenu kategórie/skupiny pridáme nižšie po načítaní dát tímu
         ...
         // Logika pre režim "Vytvoriť" (create)
             ...
             // Odstrániť listenery z iných módov, ak boli pridané
             if (unassignedClubSelect) unassignedClubSelect.onchange = null;
              // Listenery na zmenu kategórie/skupiny pridáme nižšie
             ...
             // Tieto atribúty sa nastavujú dynamicky listenermi vyššie
     ...
     // --- Režim modálu (filtračná časť) ---
         ...
         let filterTitle = 'Filter'; // Predvolený titulok

         // Nastaviť titulok modálu na základe typu filtra
         ...
         if (filterSelect) {
             ...
             // Predvybrať aktuálne aplikovaný filter (ak existuje)
             // currentFilter uchováva len POSLEDNÝ aplikovaný filter
             ...
             filterSelect.onchange = () => {
                 ...
                 // Aktualizovať aktuálny stav filtra
                 currentFilter = { type: filterType, value: selectedValue };

                 // Zatvoriť modál a obnoviť zobrazenie tabuľky s aplikovaným filtrom
                 closeModal(clubModal);
                 // resetClubModal(); // Neresetujeme celý modál, aby filter zostal nastavený pre displayCreatedTeams
                 displayCreatedTeams(); // Obnoviť zobrazenie tabuľky s novým filtrom
             };
     ...
     // Spracovanie neplatného režimu
          ...
          closeModal(clubModal); // Zavrieť modál v prípade chyby
...
if (clubForm) {
     clubForm.addEventListener('submit', async (event) => {
         ...
         // V Assign móde sa categoryId získa zo selectu nepriradených tímov
         const selectedCategoryIdInModal = currentClubModalMode === 'assign' && unassignedClubSelect && unassignedClubSelect.options[unassignedClubSelect.selectedIndex] && unassignedClubSelect.value !== '' ? unassignedClubSelect.options[unassignedClubSelect.selectedIndex].dataset.categoryId : (clubCategorySelect ? clubCategorySelect.value : null);
         const selectedGroupIdInModal = clubGroupSelect ? clubGroupSelect.value : null; // ID vybranej skupiny
         // Parsovať poradie na číslo, ak je vyplnené a select skupiny nie je prázdny, inak null
         let orderInGroup = (orderInGroupInput && orderInGroupInput.value !== '' && selectedGroupIdInModal && selectedGroupIdInModal !== '' && !selectedGroupIdInModal.startsWith('--')) ? parseInt(orderInGroupInput.value, 10) : null;
         ...
         try {
             ...
             if (operationType === 'create') {
                 ...
                 if (!selectedCategoryIdInModal || selectedCategoryIdInModal === '' || selectedCategoryIdInModal.startsWith('--')) { alert("Vyberte platnú kategóriu."); if (clubCategorySelect) clubCategorySelect.focus(); return; }
                 ...
                 const categoryNameForId = selectedCategory ? selectedCategory.name || selectedCategory.id : selectedCategoryIdInModal; // Použiť názov alebo ID kategórie
                 const newDocumentId = `${categoryNameForId} - ${clubName}`; // Formát ID: "Názov Kategórie - Názov Tímu"
                 ...
                 dataToSave = {
                     ...
                     groupId: selectedGroupIdInModal && selectedGroupIdInModal !== '' && !selectedGroupIdInModal.startsWith('--') ? selectedGroupIdInModal : null, // Uložiť groupId alebo null
                     // Uložiť poradie len ak je vybraná skupina a poradie je platné číslo > 0
                     orderInGroup: (selectedGroupIdInModal && selectedGroupIdInModal !== '' && !selectedGroupIdInModal.startsWith('--') && typeof orderInGroup === 'number' && orderInGroup > 0) ? orderInGroup : null,
             ...
             // Režim PRIRADIŤ nepriradený tím
                 ...
                 if (!unassignedClubSelect || !unassignedClubSelect.value || unassignedClubSelect.value.startsWith('--')) {
                     ...
                     // if (unassignedClubSelect) unassignedClubSelect.focus(); // Focus na zakázaný element nemá zmysel
                     return;
                 }
                 // Validácia pre skupinu a poradie
                 if (!selectedGroupIdInModal || selectedGroupIdInModal === '' || selectedGroupIdInModal.startsWith('--')) {
             ...
             // Režim UPRAVIŤ existujúci tím
                 ...
                 const newSelectedCategoryId = selectedCategoryIdInModal && selectedCategoryIdInModal !== '' && !selectedCategoryIdInModal.startsWith('--') ? selectedCategoryIdInModal : null; // Nová vybraná kategória (alebo null)
                 const newSelectedGroupId = selectedGroupIdInModal && selectedGroupIdInModal !== '' && !selectedGroupIdInModal.startsWith('--') ? selectedGroupIdInModal : null; // Nová vybraná skupina (alebo null)
                 ...
                 // ID dokumentu je "Názov Kategórie - Názov Tímu" (ak existuje kategória) alebo len "Názov Tímu"
                  ...
                  const categoryNameForNewId = categoryForNewId ? categoryForNewId.name || categoryForNewId.id : newSelectedCategoryId; // Použiť názov NOVEJ vybranej kategórie alebo jej ID


                 let potentialNewDocumentId;
                 // Ak je vybraná platná kategória, ID dokumentu je v tvare "Názov kategórie - Názov tímu"
                 if (newSelectedCategoryId) { // Ak je selectedCategoryIdInModal platný
                 ...
                 dataToSave = {
                     ...
                     categoryId: newSelectedCategoryId, // Uložiť novú kategóriu (alebo null)
                     groupId: newSelectedGroupId, // Uložiť novú skupinu (alebo null)
                     orderInGroup: newOrderInGroup, // Uložiť nové poradie (alebo null)
...
async function displayCreatedTeams() {
    ...
    // Nastaviť header tabuľky, ak je prázdny, a pridať listenery pre filter
    ...
    if (createdTeamsTableHeader.innerHTML.trim() === '') {
        createdTeamsTableHeader.innerHTML = `
            ...
            <th>Akcie</th>
        `;
        addHeaderFilterListeners(); // Pridať listenery až po vytvorení th elementov
    } else {
         // Ak header už existuje, netreba ho znovu vytvárať ani pridávať listenery
         // addHeaderFilterListeners(); // Už pridané pri prvom vytvorení headera
    }
    ...
    try {
        ...
        if (allTeams.length === 0) {
             ...
             if (createdTeamsTableHeader.innerHTML.trim() === '') {
                  createdTeamsTableHeader.innerHTML = `
                       ...
                       <th>Akcie</th>
                   `;
                   addHeaderFilterListeners(); // Pridať listenery aj tu
             }
        ...
        // --- Aplikácia filtra ---
        teamsToDisplay = allTeams; // Začať s celým zoznamom tímov (ktorý je už zoradený)

        // Ak je aktívny filter (typ a hodnota nie sú null)
        if (currentFilter.type && currentFilter.value !== null) {
            console.log(`Aplikujem filter: Typ=${currentFilter.type}, Hodnota=${currentFilter.value}`);
             ...
             // Hodnota filtra prevedená na lowercase a orezaná pre case-insensitive a whitespace-agnostic porovnanie
            const filterValueLowerTrimmed = typeof currentFilter.value === 'string' ? currentFilter.value.trim().toLowerCase() : currentFilter.value;


            // Filtrovať tímy na základe typu a hodnoty filtra
            teamsToDisplay = allTeams.filter(team => { // Filter aplikovať na ZORADENÝ allTeams
                // Prevod hodnôt tímu na lowercase a orezanie pre porovnanie
                 ...
                 const teamCategoryId = team.categoryId; // ID kategórie
                 const teamGroupId = team.groupId; // ID skupiny
            ...
            console.log("Žiadny aktívny filter, zobrazujem všetky tímy (už zoradené abecedne):", allTeams.length);
            // Ak nie je filter, teamsToDisplay je rovné allTeams (už zoradené abecedne vyššie)
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // displayCreatedTeams() načíta tímy a nastaví počiatočné teamsToDisplay = allTeams
    await displayCreatedTeams();


    // Pridajte listenery na hlavičky tabuľky po prvom zobrazení dát (alebo ak header už je v HTML)
    // addHeaderFilterListeners() sa teraz volá v displayCreatedTeams ak je header prázdny
    // Ak header je v HTML napevno, volajte ju tu:
    ...
    // Konfigurácia tlačidla "+" pre túto sekciu (Zoznam tímov)
     ...
     if (clubModalClose) {
         clubModalClose.addEventListener('click', () => {
              ...
              // Po zatvorení modalu klubu, obnoviť tabuľku s AKTUÁLNYM filtrom
              displayCreatedTeams();
         });
     }
     ...
     if (clubModal) {
         window.addEventListener('click', (event) => {
             // Ak bol kliknutý cieľ samotný modál (pozadie), ktorý nie je obsah modálu
              if (event.target === clubModal && event.target.querySelector('.modal-content') && !event.target.querySelector('.modal-content').contains(event.target)) {
                 closeModal(clubModal);
                 resetClubModal(); // Resetovať po zatvorení
                 // Po zatvorení modalu klubu, obnoviť tabuľku s AKTUÁLNYM filtrom
                 displayCreatedTeams();
             } else if (event.target === clubModal) {
                  // Ak je kliknutý presne element s id="clubModal" (pozadie)
                  closeModal(clubModal);
                  resetClubModal(); // Resetovať po zatvorení
                  // Po zatvorení modalu klubu, obnoviť tabuľku s AKTUÁLNYM filtrom
                  displayCreatedTeams();
...
// export { allAvailableCategories, allAvailableGroups, allTeams, teamsToDisplay };
